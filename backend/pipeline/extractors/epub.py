...
async def extract_epub(filepath: str) -> ExtractedContent:
    logger.info(f"Extracting EPUB from {filepath}")
    path = Path(filepath)
    if not path.exists():
    ...
    full_text = "\n\n---\n\n".join(chapters)
    logger.info(f"Extracted {len(chapters)} chapters, {len(full_text.split())} words from '{title}'")
